...
export interface AuthInitFailedAction extends Action {
    readonly type: typeof AUTH_INIT_FAILED;
}
...
export const refreshTokenActionCreator: ActionCreator<ThunkAction<void, void, void, AnyAction>> = () => {
    return async (dispatch: Dispatch) => {
        API({
            ...
            dispatch({ type: 'AUTH_INIT_FAILED' });  // 👈 trigger reducer fallback
        });
    };
};
